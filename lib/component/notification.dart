...
class _NotificationNewsCard extends StatefulWidget {
  final News news;
  final bool isActive;
  final bool isClickedNews;

  const _NotificationNewsCard({
    required this.news,
  ...
  @override
  State<_NotificationNewsCard> createState() => _NotificationNewsCardState();
}

class _NotificationNewsCardState extends State<_NotificationNewsCard> {
  ...
  @override
  void didUpdateWidget(_NotificationNewsCard oldWidget) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && !_hasBeenMarkedAsRead) {
          ...
          if (!newsProvider.isRead(widget.news)) {
            ...
            print('📖 [Notification] Marked article as read: ${widget.news.title}');
          }
        }
      });
    }
