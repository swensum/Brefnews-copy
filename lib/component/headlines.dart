...
class _HeadlinesDetailPageState extends State<HeadlinesDetailPage>
    ...
    try {
      ...
      if (widget.headline.containsKey('news_articles') &&
        ...
        if (newsList.isNotEmpty) {
          print(
            '🟢 [HeadlinesDetail] Found ${newsList.length} articles in headline data',
          );
          if (mounted) {
            setState(() {
      ...
      print(
        '🟢 [HeadlinesDetail] Fallback found ${relatedNews.length} articles',
      );
  ...
  bool _doesHeadlineMatch(String newsHeadline, String timelineHeadline) {
    ...
    final cleanNews = newsHeadline
        .replaceAll(RegExp(r'[^\w\s]'), '')
        .toLowerCase()
        .trim();
    final cleanTimeline = timelineHeadline
        .replaceAll(RegExp(r'[^\w\s]'), '')
        .toLowerCase()
        .trim();
    ...
    if (cleanNews.contains(cleanTimeline) ||
        cleanTimeline.contains(cleanNews)) {
      return true;
    }

    // Strategy 4: Split into words and check for significant overlap
    final newsWords = cleanNews
        .split(' ')
        .where((word) => word.length > 3)
        .toSet();
    final timelineWords = cleanTimeline
        .split(' ')
        .where((word) => word.length > 3)
        .toSet();
    ...
    return Consumer<NewsProvider>(
      builder: (context, newsProvider, child) {
        ...
        if (_headlineNews.length == 1) {
          return _buildSingleNews(newsProvider, _headlineNews.first);
        }
    ...
    return Center(
      child: Padding(
        ...
        child: _HeadlineNewsCard(
          ...
          isActive: true,
          isClickedNews:
              widget.initialNews != null && news.id == widget.initialNews!.id,
        ),
      ),
    );
  }
...
// ========== CHANGED: Converted to StatefulWidget with read marking logic ==========
class _HeadlineNewsCard extends StatefulWidget {
  final News news;
  final bool isActive;
  final bool isClickedNews;

  const _HeadlineNewsCard({
    required this.news,
  ...
  @override
  State<_HeadlineNewsCard> createState() => _HeadlineNewsCardState();
}

class _HeadlineNewsCardState extends State<_HeadlineNewsCard> {
  ...
  @override
  void didUpdateWidget(_HeadlineNewsCard oldWidget) {
  ...
  // ========== ADDED: Mark as read when active ==========
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && !_hasBeenMarkedAsRead) {
          ...
          if (!newsProvider.isRead(widget.news)) {
            ...
            print('📖 [Headline] Marked article as read: ${widget.news.title}');
          }
        }
      });
    }
                            ...
                            child: Row(
                              ...
                              children: [
                                Consumer<NewsProvider>(
                                  builder: (context, newsProvider, child) {
                                    bool isBookmarked = newsProvider
                                        .isBookmarked(widget.news);
                                    return GestureDetector(
